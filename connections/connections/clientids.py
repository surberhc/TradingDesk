...
CLIENT_IDS = {
    ...
    "dashboard_s8": 52,              # RETIRED 2026-08-24 with dashboard/app.py (now dashboard/archive/app_8501.py); the :8502 rebuild connects as "desk_s8" (64) instead. Kept reserved rather than reused so an old process can never collide. Was: app.py's S8 monitor tab -- READ-ONLY (connects readonly=True) to the live-TRADING Gateway (port 4003, connections.ibkr_live_trade), launch=False (never boots the Gateway from a passive auto-refreshing web page), short timeout. Targeted per-pick quote pulls (2 legs at a time) + accountSummary() read, purely for DISPLAY re-marking of today's logged picks -- never builds or transmits an order. Own id so it never collides with the runner's live-cycle connection s8_live_pilot (54) if both are polling port 4003 at the same moment
    ...
    "paperbot_live_fa_block_exec": 62,  # paperbot FA-GROUP BLOCK EXECUTOR (paperbot/live_fa_block_execute.py): the advisor-account/FA-group block-execution path — reads per-account target shares from the frozen rebalance_engine, sets a tier GROUP's ContractsOrShares via replaceFA (mandatory backup + written-XML diff in the unarmed preview + preserve-other-groups + fail-closed-on-blank), and places ONE FA block (faMethod="", faGroup=<group>) behind the review->arm->transmit gate. PREVIEW BY DEFAULT (transmits nothing without the exact --arm-i-understand token + a physically-armed gateway probe). Built + VALIDATED against the PAPER advisor master DF8922141 (port 4002, subs DU8922142-146); connects readonly=False ONLY on the armed path, PINNED to a DU sub (never the master DF...141 — its account stream hangs). Own id so it never collides with the paper engine (30), the read-only rebalance runner (37), or the transmit-capable per-account rebalance EXECUTOR (38). NEVER whatIf a block (it hangs)
    "desk_s8": 64,                   # desk dashboard :8502 S8 page (dashboard/desk/page_s8.py::_s8_connect_readonly_short): READ-ONLY, DISPLAY-ONLY connection to the live-TRADING Gateway (port 4003, connections.ibkr_live_trade), launch=False (an auto-refreshing page must never boot a Gateway), short timeout. Reads accountSummary() for the account/margin panel and connection health -- never builds or transmits an order. ADDED 2026-08-24: page_s8.py had called connect("desk_s8") since the rebuild, but the name was never registered, so clientids.get() raised KeyError and the panel always rendered as a connection failure. New id rather than reusing the retired 52, so a stale app_8501.py process could never collide.
    "live_fa_block_exec": 63,        # RESERVED (not built): the future LIVE-TRADE (port 4003) FA-group block executor consumer — the same live_fa_block_execute module pointed at the live advisor login once the owner PROVISIONS it (the live 4003 login is NOT yet an advisor account: tested 2026-08-05 — 2 direct accounts, no master, requestFA times out). Reserved now (mirroring the s4 44/45, s8 49/50, s0 57/58 reserved-exec pattern) so the paper build consumer (62) and the future live consumer never collide when the target gateway is flipped from PAPER to LIVE. Nothing transmits on 4003 today
}
