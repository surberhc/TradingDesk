...
level. That is backstop #1. Backstop #2 lives in this file: `connect()` has no
`readonly` parameter at all and always connects read-only, and this module never
...
# tests can point it at a tmp dir. Distinct path AND distinct env var from the
# paper module's lock — the two must never share a lockfile.
GATEWAY_LAUNCH_LOCK = os.environ.get(
    "TRADINGDESK_LIVE_DATA_GATEWAY_LAUNCH_LOCK",
    r"C:\TradingDesk-Local\state\live_data\gateway_launch.lock",
)
RELAUNCH_COOLDOWN_SECS = 180   # no relaunch within this long of the last attempt


def gateway_running(client_id: int = clientids.CLIENT_IDS["live_data_forward"],
                    timeout: int = 8) -> bool:
    """True if the live-data Gateway is up and serving data (a real data round-trip).

    Connects read-only (see connect() below for why that's not a parameter here).
    """
    ib = IB()
    try:
        ib.connect(HOST, LIVE_DATA_PORT, clientId=client_id, readonly=True, timeout=timeout)
...
def ensure_gateway(wait_secs: int = 180) -> bool:
    """Make sure the live-data Gateway is up; launch it (IBC auto-login) if not.
...
def connect(consumer: str, launch: bool = False, timeout: int = 10) -> IB:
    """Connect to the LIVE-side read-only data Gateway using a registered clientId.

    consumer : a key in connections.clientids.CLIENT_IDS (e.g. "live_data_forward").
    launch   : if True, start the Gateway first when it's down.

    Deliberately has NO `readonly` parameter. Every connection this module ever
    makes is read-only, hardcoded below — not passed through, not overridable by
    any caller. That is the load-bearing safety property of this whole module: even
    if a future caller wanted a writable session, there is no argument here that
    could grant one. Do not "fix" this by adding a readonly param back.
    """
    client_id = clientids.get(consumer)
    if launch and not gateway_running():
        if not ensure_gateway():
            raise RuntimeError("live-data Gateway did not come up")
    ib = IB()
    ib.connect(HOST, LIVE_DATA_PORT, clientId=client_id, readonly=True, timeout=timeout)
    return ib
