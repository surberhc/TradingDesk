r"""
collect_spx_1m.py — bulletproof, resumable collector for SPX-ROOT 1-minute option data.

Sibling of collect_spxw_1m.py, pointed at the SPX ROOT (the AM-settled monthly
options) instead of the SPXW weeklies/0DTE. Byte-for-byte the SAME LOGIC; only the
SYMBOL, the output tree, and the sidecar (progress/log/known-empty) paths differ so
it can NEVER collide with — or clobber the heartbeat of — the running SPXW collector.

Pulls 1-minute NBBO QUOTE and 1-minute OHLC bars for every SPX-root contract, one
trading day at a time, from the LOCAL ThetaData v3 Terminal.

Why two endpoints, pulled differently (MEASURED on SPXW, confirmed on SPX-root):
  * QUOTE  /option/history/quote  ACCEPTS expiration=*  -> ONE call/day.
  * OHLC   /option/history/ohlc   REJECTS  expiration=*  -> must LOOP per
           expiration. The day's expiration list comes from
           /option/history/greeks/eod (expiration=*).
  CRITICAL: v3 requires DASHED dates (start_date=2026-06-25), NOT 20260625.
  Valid interval token = "1m".

Output tree (NEW, never collides with the SPXW tree or the EOD warehouse):
    C:\TradingDesk-Local\warehouse\raw\options_1m\SPX\quote\{YYYYMMDD}.parquet
    C:\TradingDesk-Local\warehouse\raw\options_1m\SPX\ohlc\{YYYYMMDD}.parquet
...
Bulletproof / resumable contract:
  ...
  * Order: newest day -> oldest (recent data lands first / usable soonest).
...
This module is fully runnable on its own and is the unit of resumability. It is NOT
supervised by spxw_1m_supervisor.py (that watchdog is bound to the SPXW instance);
this one is launched directly and re-launched with the same command to resume.
"""
...
# --------------------------------------------------------------------------- #
# Paths — a NEW tree so nothing collides with the SPXW tree or the EOD warehouse.
# --------------------------------------------------------------------------- #
ROOT_1M = config.DATA_ROOT / "raw" / "options_1m" / "SPX"
QUOTE_DIR = ROOT_1M / "quote"
OHLC_DIR = ROOT_1M / "ohlc"
PROGRESS = config.DATA_ROOT / "spx_1m_progress.json"
LOG = config.DATA_ROOT / "spx_1m.log"
KNOWN_EMPTY = config.DATA_ROOT / "spx_1m_known_empty.json"
...
def load_known_empty() -> set[str]:
    ...
    Persisted by the warehouse audit (spx_1m_known_empty.json). These days never
    return data, so re-probing them writes no file — which a file-count watchdog
    reads as a stall, so the collector could never reach its clean 'done' exit.
    Excluding them lets a full pass find nothing pending and exit 0. A
    missing/corrupt file -> empty set (old behavior). Only known closures are
    listed, never a real trading day, so this can never mask a genuine data hole.
    """
    try:
...
def pull_quote(d: dt.date) -> pd.DataFrame:
    """1-min NBBO for ALL SPX-root contracts that day — ONE call (expiration=*)."""
...
def collect_day(d: dt.date) -> tuple[bool, int, str]:
    ...
    Writes ohlc first, then quote LAST; the day is only DONE when both land. If the
    ...
    un-done. SPX trades every weekday in-window, so empty == holiday; those are
...
# --------------------------------------------------------------------------- #
def main() -> int:
    ap = argparse.ArgumentParser(description="SPX-root 1-minute option collector")
    ...
    ap.add_argument("--progress", default=None,
                    help="override path for the progress/heartbeat JSON; default "
                         "spx_1m_progress.json. Use a DISTINCT path when running a "
                         "2nd (unsupervised) catch-up instance so it can't clobber "
                         "the primary's status / any staleness alarm.")
    ap.add_argument("--log", default=None,
                    help="override path for the append-only log; default spx_1m.log. "
    ...
    # so it writes its own status + log and cannot corrupt the primary's files. Omitting
    # the flags = default behavior. write_progress()/log() reference these MODULE
    # globals, so reassigning them here (before the loop) redirects all subsequent writes.
    global PROGRESS, LOG
    if args.progress:
    ...
    # pass shows 100% and the run can exit cleanly instead of stall-looping.
    ...
    # as done to avoid an infinite re-launch loop on market holidays).
    if not remaining:
    ...
    if collected_this_run == 0 and args.max_days == 0:
        ...
        # return no data (holidays / pre-listing). Treat as complete so a
        # re-launch stops cleanly rather than spinning forever.
        ...
        return 0
    log("pass ended with days still to do — exiting 1 so a re-launch resumes")
    return 1
