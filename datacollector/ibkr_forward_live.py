...
Design choices that keep it safe alongside other Gateway clients:
  * Its OWN clientId (`live_data_forward` = 48, see connections.clientids). Distinct
    from ibkr_forward.py's `datacollector_forward` (25) and from every paperbot id,
    so none of them ever collide.
  * Read-only by construction (connections.ibkr_live_data has no writable path).
...
Usage:
    ...
    <venv python> ibkr_forward_live.py SPY QQQ           # full chains for these roots, today
    <venv python> ibkr_forward_live.py                   # full universe (config.all_roots()), today
"""
