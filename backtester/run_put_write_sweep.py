r"""
run_put_write_sweep.py -- THIN, RESUMABLE runner for the index-level PUT-WRITE structure.
...
Checkpoints: one JSON per cell under output/s5_financing/put_write/cells/ (gitignored).
...
OUT_DIR = Path(__file__).resolve().parent / "output" / "s5_financing" / "put_write"
...
def main() -> None:
    ...
    print(f"[start] put_write sweep: {len(combos)} combos x {len(WINDOWS)} windows "
