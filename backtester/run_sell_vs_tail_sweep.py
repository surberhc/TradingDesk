r"""
run_sell_vs_tail_sweep.py -- THIN, RESUMABLE runner for the SELL-AGAINST-OWNED-TAIL structure.
...
STRUCTURE: sw.sell_against_owned_tail_spec() with defaults tail_moneyness=-0.20, tail_dte=63
-- these are FIXED = S5's validated always-on Tier-1 tail (deep ~20% OTM, ~63 DTE) and are
NOT swept. Only the SHORT leg's tenor/delta/management/regime are swept. Income = the SHORT
leg's net premium as net_pct_yr_of_core; the deep tail's own carry is NOT counted (it is the
~1.56%/yr being funded -- no double-count). This is a MULTI-EXPIRY structure (short at the
swept front tenor, tail at ~63 DTE); with per-leg-expiry settlement (harness commit 06faee2)
each leg settles on its own expiry honestly.

Grid (full pre-registered space over the SHORT leg, unchanged from the driver):
    tenor_dte  : {7, 14, 30, 45}     (the SHORT leg tenor; tail stays fixed at 63 DTE)
...
Checkpoints: one JSON per cell under output/s5_financing/sell_vs_tail/cells/ (gitignored).
...
# STRUCTURE = sell-against-owned-tail with FIXED tail (tail_moneyness=-0.20, tail_dte=63 =
# S5's validated always-on Tier-1 tail). The tail moneyness/DTE are NOT swept; only the SHORT
# leg's tenor/delta/management/regime are swept.
SPEC = sw.sell_against_owned_tail_spec()
...
OUT_DIR = Path(__file__).resolve().parent / "output" / "s5_financing" / "sell_vs_tail"
