...
- Real VIX is more cautious than the realized-vol proxy → deeper de-risking: drawdowns improved, CAGR/Sortino lower (honest trade-off; favors the smoothness mandate). `FRED_API_KEY` is now present in `C:\TradingDesk-Local\secrets\.env`; verified authenticating 2026-06-28; FRED HY OAS (`BAMLH0A0HYM2`) returns only 2023-06-27→2026-06-25 (~3 yrs) due to the April-2026 ICE rolling-3-year restriction, so the real OAS is available for recent dates only (~2023+) and the HYG/IEF proxy is retained for history.
...
- **Tried to upgrade credit from proxy to real ICE BofA HY OAS via FRED** (`BAMLH0A0HYM2`). `FRED_API_KEY` is now present in `C:\TradingDesk-Local\secrets\.env` and verified authenticating 2026-06-28, but FRED returns **only ~3 years** (795 obs, 2023-06-27→2026-06-25): ICE **restricted its indices to a rolling 3-year window in April 2026**. Full history is now commercial-only (ICE/Bloomberg/Refinitiv). So real OAS is NOT wireable here — reverted the partial parquet, manifest stays `proxy`. The IBKR API doesn't carry this index either.
