...
| `paperbot/cashflows.py` | Owns the per-account `SCHEDULE` (currently empty) + `reserve_for()` = `RESERVE_MONTHS (=2) × monthly distributions`. The "reserve=0 unless a cashflow is scheduled" hook already exists here. |
...
- **How many months to hold:** `RESERVE_MONTHS = 2` is the current deliberate default (Andrew's call, 2026-09-04: hold a thicker buffer — 1 month is the never-caught-short minimum, 2 months trades a bit of idle cash for fewer forced rebalance sales and less paid in trading fees). Spec a **buffer option**: hold a flat floor always, optionally a date-proximity ramp (reserve grows as the distribution day approaches) so capital isn't idle all month; ramp is a later refinement.
