...
### 3.10 Fractional / cash-quantity trading — NOT available on the TWS API (crypto/forex only) — **VERIFIED 2026-07-27**

The TWS socket API this desk uses (`ib_async` 2.1.0, all three Gateways on ports 4001/4002/4003) does **not** support fractional-share or cash-quantity (`cashQty`) orders for stocks or ETFs. IBKR's current *Notes & Limitations → Introduction* page states, verbatim (accessed 2026-07-27):
...
(the "Interacive" typo is IBKR's own.) Source: https://www.interactivebrokers.com/docs/general/order-types/notes-limitations/introduction — **VERIFIED** 2026-07-27 by reading the page directly in-session.

Consequences for the desk:
- `Order.totalQuantity` and `Order.cashQty` accept decimals at the **library** level (`ib_async` 2.1.0 types `totalQuantity: float` and `cashQty: float | Decimal`), so a decimal will be sent — but the IBKR **backend** rejects/rounds fractional equity; the block is server-side, not the library. On the TWS API `cashQty` is usable only for **Forex** (denominate in the pair's 2nd currency) and **Crypto** (MKT). — **RECORDED** (2026-07-27 research: ib_async order.py source + maintainer discussion #184).
- Our whole-share integer sizing — `reconcile.py` `target_shares = int(weight * investable / price)` — is therefore a **hard constraint of the stack**, not something config can lift. A small account cannot hold expensive ETFs: surfaced by the S0 live pilot 2026-07-27 (account U5721712 at $957 NetLiq → SPY $737 / VTI $364 both size to 0 whole shares).
- The **only** programmatic path to fractional stock buying at IBKR is the **Web API (Client Portal API / CPAPI)** — a *separate* API from the TWS socket API — which added stock `cashQty` on **2026-03-27** (IBKR Web API changelog). Adopting it would mean a second, parallel order path (different session-auth model, new dependency), **single-account only** (FA group/allocation orders over the TWS API cannot produce fractional fills). — **RECORDED** (2026-07-27 research; a direct fetch of the campus Web API changelog returned HTTP 403, so this is not personally re-read — flag for manual re-verification if CPAPI is ever pursued).
