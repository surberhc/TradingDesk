...
# recon_report owns the ONE held-aside pricing reader (_portfolio_values) — the same one the
# block rail uses. Imported, never re-implemented.
import recon_report
import s0_live
...
def _run_session(ib, target, *, armed: bool, conform: bool,
    ...
    # HELD-ASIDE classification input (v0.41.0): the broker's OWN contract.secType ("STK",
    # "BOND", ...), never a guess off the ticker text. The same input the block rail and the
    # batch rail already pass. Without it holding_class.carve_out short-circuits to "nothing
    # held aside" and an individual bond is sized INSIDE the target allocation — contrary to
    # owner decision D6, which puts held-aside holdings OUTSIDE it and applies the model to
    # the remaining sleeve as its own 100%.
    ...
    # — a stale daily close is not a price this rail can trade at.
    ...
    # is UNTRACKED, an ALWAYS-BREACH status that sizes as delta = 0 - held. This is the rail
    # that has traded REAL money on the funded trust account — it refuses instead.
    ...
    # the holding UNPRICED and withholds orders — fail closed, not a silent zero).
